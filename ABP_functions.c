...
void ReadInputParameters(int argc, char* argv[], char** command_line_output, param* parameters, inputparam* input_parameters){
    ...
    *command_line_output = malloc(1000);
    ...
    strcat(*command_line_output, "kappa ");
    number_of_input_parameters++;
    ...
    strcat(*command_line_output, "D_0 ");
    number_of_input_parameters++;

    strcat(*command_line_output, "delta_D ");
    number_of_input_parameters++;
    ...
    parameters[0].kappa = strtod(argv[i], NULL); i++;
#ifdef QSAP
    parameters[0].rho_m = strtod(argv[i], NULL); i++;
    parameters[0].D_0 = strtod(argv[i], NULL); i++;
    parameters[0].delta_D = strtod(argv[i], NULL); i++;
...
void StoreInputParameters(int argc, char* argv[], param parameters, inputparam input_parameters, char* command_line_output){
    ...
    */
    ...
    #elif defined QSAP
    double D_variation = parameters.delta_D/parameters.D_0;
    fprintf(parameters.param_file, "Rho = %lf \t delta_D/D_0 = %lf \n", rho, D_variation);
...
double DensityDependentDiffusion(double rho, double rho_m, double phi, double D_0, double delta_D){
    /*
    Give the density-dependent speed (formula in the MIPS review paper)
    */
    return D_0 + delta_D/2 * tanh((rho-rho_m)/phi);
}

double ForcePolymer(double r, double kappa){
    /*
    Give the Hookean force at distance r
    */
    return -kappa*r;
}
...
void MeasureDensityAndForce(long* neighbors, particle* particles, long** boxes, param* parameters, box*** neighboring_boxes) {
    ...
    */
    ...
    double Z = NORMALIZATION_EXP * rmax_squared;
    double kappa = parameters[0].kappa;
    #ifdef PFAP
    ...
    for (i=0; i<parameters[0].N; i++) {
        ...
        #ifdef WALL
        particles[i].fwx = ForceWall(particles[i].x, wall_size, Lx, omega);
        particles[i].fx = particles[i].fwx;
        particles[i].fwy = ForceWall(particles[i].y, wall_size, Ly, omega);
        particles[i].fy = particles[i].fwy;
        #else
        particles[i].fx = 0;
        particles[i].fy = 0;
        #endif
    ...
    // Calculate the polymer forces
    for (i=1; i<parameters[0].N; i++) {
        dx = particles[i].x - particles[i-1].x;
        dy = particles[i].y - particles[i-1].y;
        particles[i].fx += ForcePolymer(dx, kappa);
        particles[i].fy += ForcePolymer(dy, kappa);
        particles[i=1].fx -= ForcePolymer(dx, kappa);
        particles[i=1].fy -= ForcePolymer(dy, kappa);
    }
    ...
    for (i=0;i<parameters[0].N;i++) {
        particles[i].Dr = DensityDependentDiffusion(particles[i].rho, parameters[0].rho_m, parameters[0].phi, parameters[0].D_0, parameters[0].delta_D);
...
void PolymerInitialConditions(FILE* input, particle* particles, param* parameters, double *t
                        #ifdef HASHING
                        , long*** boxes, long** neighbors
                        #endif
                        ){
    /*
    Initial conditions of a polymer.
    */
}
...
void UpdateParticles(particle* particles, param* parameters, double* step
    ...
    */ 
    ...
    for(i = 0 ; i < parameters[0].N ; i++) {
        ...
        // printf("v %lf \n", particles[i].v);
        // printf("rho %lf \n", particles[i].rho);    
        if (fabs(particles[i].fx) > fmax) fmax = fabs(particles[i].fx);
        if (fabs(particles[i].fy) > fmax) fmax = fabs(particles[i].fy);
    }
    // printf("fmax %lf \n", fmax);
    ...
    for (i=0;i<parameters[0].N;i++){
        ...
        particles[i].x += step[0] * particles[i].fx + sqrt(2 * step[0] * particles[i].Dr) * gasdev();
        particles[i].y += step[0] * particles[i].fy + sqrt(2 * step[0] * particles[i].Dr) * gasdev();

        // PBC should not come into play as the walls are usually there
        if (particles[i].x>=Lx) particles[i].x -= Lx;
        if (particles[i].x<0) particles[i].x += Lx;
        if (particles[i].y>=Ly) particles[i].y -= Ly;
        if (particles[i].y<0) particles[i].y += Ly;
